...
neoc_neo_block_t* neoc_neo_block_clone(const neoc_neo_block_t* block) {
    ...
    if (block->transaction_count > 0 && block->transactions) {
        ...
        for (size_t i = 0; i < block->transaction_count; i++) {
            ...
            if (block->transactions[i]) {
                neoc_error_t clone_err = neoc_transaction_clone(block->transactions[i], &clone->transactions[i]);
                if (clone_err != NEOC_SUCCESS) {
                    clone->transactions[i] = NULL;
                }
                if (!clone->transactions[i]) {
                    // Clean up on failure
                    for (size_t j = 0; j < i; j++) {
                        if (clone->transactions[j]) {
                            neoc_transaction_free(clone->transactions[j]);
                        }
                    }
                    free(clone->transactions);
                    neoc_neo_block_free(clone);
                    return NULL;
                }
            } else {
...
neoc_hash256_t neoc_neo_block_calculate_merkle_root(const neoc_neo_block_t* block) {
    ...
    if (block->transaction_count == 1) {
        ...
        neoc_error_t err = neoc_transaction_calculate_hash(block->transactions[0], &tx_hash);
        if (err == NEOC_SUCCESS) {
            return tx_hash;
        }
        return root;
    ...
    for (size_t i = 0; i < block->transaction_count; i++) {
        neoc_error_t err = neoc_transaction_calculate_hash(block->transactions[i], &hashes[i]);
        if (err != NEOC_SUCCESS) {
            // Set to zero hash on error
            memset(&hashes[i], 0, sizeof(neoc_hash256_t));
        }
    }
...
uint8_t* neoc_neo_block_serialize(const neoc_neo_block_t* block, size_t* out_length) {
    ...
    size_t total_size = sizeof(uint32_t) + 32 + 32 + sizeof(uint64_t) + 
                       sizeof(uint64_t) + sizeof(uint32_t) + sizeof(uint8_t) + 20;
    
    // Add witness size if present
    if (block->header.witness) {
        total_size += 4 + block->header.witness->invocation_script_len + 
                     block->header.witness->verification_script_len;
    }
    
    // Add transaction sizes
    total_size += 4; // transaction count
    for (size_t i = 0; i < block->transaction_count; i++) {
        // Simplified: assume 1KB per transaction
        total_size += 1024;
    }
    ...
    memcpy(buffer + offset, &block->header.timestamp, sizeof(uint64_t));
    offset += sizeof(uint64_t);
    
    memcpy(buffer + offset, &block->header.nonce, sizeof(uint64_t));
    offset += sizeof(uint64_t);
    
    memcpy(buffer + offset, &block->header.index, sizeof(uint32_t));
    offset += sizeof(uint32_t);
    
    memcpy(buffer + offset, &block->header.primary_index, sizeof(uint8_t));
    offset += sizeof(uint8_t);
    
    memcpy(buffer + offset, &block->header.next_consensus, 20);
    offset += 20;
    
    // Write transaction count (simplified)
    uint32_t tx_count = (uint32_t)block->transaction_count;
...
neoc_neo_block_t* neoc_neo_block_deserialize(const uint8_t* data, size_t length) {
    if (!data || length < 169) { // Minimum header size
        return NULL;
    }
    ...
    // Read header fields
    memcpy(&block->header.version, data + offset, sizeof(uint32_t));
    offset += sizeof(uint32_t);
    
    memcpy(&block->header.prev_hash, data + offset, 32);
    offset += 32;
    
    memcpy(&block->header.merkle_root, data + offset, 32);
    offset += 32;
    
    memcpy(&block->header.timestamp, data + offset, sizeof(uint64_t));
    offset += sizeof(uint64_t);
    
    memcpy(&block->header.nonce, data + offset, sizeof(uint64_t));
    offset += sizeof(uint64_t);
    
    memcpy(&block->header.index, data + offset, sizeof(uint32_t));
    offset += sizeof(uint32_t);
    
    memcpy(&block->header.primary_index, data + offset, sizeof(uint8_t));
    offset += sizeof(uint8_t);
    
    memcpy(&block->header.next_consensus, data + offset, 20);
    offset += 20;
    
    // Read transaction count if there's more data
    if (offset + 4 <= length) {
        uint32_t tx_count;
        memcpy(&tx_count, data + offset, sizeof(uint32_t));
        offset += sizeof(uint32_t);
        
        // Parse transactions if present
        block->transaction_count = tx_count;
        if (tx_count > 0) {
            block->transactions = calloc(tx_count, sizeof(neoc_transaction_t*));
            if (block->transactions) {
                for (uint32_t i = 0; i < tx_count && offset < length; i++) {
                    // Parse each transaction from the remaining data
                    size_t tx_size = length - offset;
                    block->transactions[i] = neoc_transaction_deserialize_simple(data + offset, tx_size, &tx_size);
                    if (block->transactions[i]) {
                        offset += tx_size;
                    } else {
                        // If parsing fails, stop and use what we have
