...
#include <string.h>
#include <stdio.h>
#include "neoc/protocol/response/notification.h"
...
neoc_notification_t* neoc_notification_from_json(const char* json_str) {
    ...
    // Get state (can be null)
    neoc_json_t* state_json = neoc_json_get_object(json, "state");
    neoc_stack_item_t* state = NULL;
    
    if (state_json) {
        // Convert state JSON to string for parsing
        char* state_str = neoc_json_to_string(state_json);
        if (state_str) {
            state = neoc_stack_item_from_json(state_str);
            neoc_free(state_str);
        }
    }
    
    // Create notification
    notification = neoc_notification_create(&contract, event_name, state);
    
    if (!notification && state) {
        neoc_stack_item_free(state);
    }
